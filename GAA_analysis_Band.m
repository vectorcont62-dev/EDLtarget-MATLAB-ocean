...
for i=1:length(r.r)
    ...
    elseif (r.r(i)>=r.r_CTN(1) && r.r(i)<r.r_CTN(N))
        ...
        V_instant_CTN(slice_num) = V_instant_CTN(slice_num-1) + KCTN(slice_num)/p.e_ox*log(r.r(i)/r.r_CTN(slice_num-1)) + q*nt_ctn(slice_num)/4/p.e_n*(r.r(i)^2-r.r_CTN(slice_num-1)^2);
