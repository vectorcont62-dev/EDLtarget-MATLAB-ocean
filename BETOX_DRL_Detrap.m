...
% nt_ctn = 0.405e19; V_ch = -0.1;% 1V, to1=2.5
...
Vg = 20;
Rad = setRadius(20e-7, 8e-7, 2e-7, 2e-7, 0.9e-7, 6e-7, 8e-7, 2e-7); % DRL Spec
...
Vol = pi*((Rad.r_tn1)^2-Rad.r_tox1^2);
...
[~, ~, ~, ~, ~,~, Vth_ini, Vth_det ] = setFieldParameters(Rad, Para, V_ch, 0, nt_tn1, nt_ctn)

% Vth_ini = nt_tn1 *Para.q*Vol/Ctg + nt_ctn * Para.q*Vol2/CNG
% Vth_det = nt_tn1 *Para.q*Vol/Ctg
% psi_surface = psi_s(Rad, Para, Vth_ini)

[E_c0, E_v0, E_field0] = GAA_analysis_Band(Rad, Para, V_ch, Vg, nt_tn1, nt_ctn); % 1us program, Vch = -1.7V
